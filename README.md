...
Our current implementation does not include Yobe's SDK. The SDK release we had access to was more compatible with a one-query, non-looping model, which we previously implemented. Since we wanted to emphasize the conversational aspect of BUtLAR, we decided to create an implementation that does not utilize the previous SDK. However, Yobe's new SDK, called BioPSI, has extensive potential for integrating a real-time audio capture. This repository serves as the groundwork for integrating BioPSI.
